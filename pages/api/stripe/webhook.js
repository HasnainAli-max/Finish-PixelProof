...
// ⬇️ Use Admin Firestore + Timestamp
import { dbAdmin as db, FieldValue, Timestamp } from '@/lib/firebase/firebaseAdmin';
...
async function writeFromSubscriptionEvent(subscription) {
  ...
  const payload = {
    ...
    cancelAt: tsOrNull(subscription.cancel_at),
    canceledAt: tsOrNull(subscription.canceled_at),
    endedAt: tsOrNull(subscription.ended_at),
