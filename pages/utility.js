...
export default function UtilityPage() {
  ...
  // ---------------- THEME (only this part changed) ----------------
  const [darkMode, setDarkMode] = useState(false);
  // Initial apply + migrate keys + set button state
  useEffect(() => {
    const root = document.documentElement;
    try {
      // prefer canonical 'theme', else 'color-theme', else legacy 'pp_dark'
      let stored =
        localStorage.getItem('theme') ||
        localStorage.getItem('color-theme') ||
        (localStorage.getItem('pp_dark') === '1'
          ? 'dark'
          : localStorage.getItem('pp_dark') === '0'
          ? 'light'
          : '') ||
        root.dataset.theme ||
        '';
      ...
      // persist canonically + legacy (backward-compat)
      localStorage.setItem('theme', stored);
      localStorage.setItem('pp_dark', stored === 'dark' ? '1' : '0');
  ...
  // Whenever darkMode changes via toggle, update DOM + both keys
  useEffect(() => {
    const root = document.documentElement;
    root.classList.toggle('dark', darkMode);
    root.dataset.theme = darkMode ? 'dark' : 'light';
    try {
      localStorage.setItem('theme', darkMode ? 'dark' : 'light'); // canonical
      localStorage.setItem('pp_dark', darkMode ? '1' : '0');      // legacy
    } catch {}
  }, [darkMode]);

  // Keep in sync with external changes (other tabs/pages)
  useEffect(() => {
    const onStorage = (e) => {
      if (!e || !e.key) return;
      if (['theme', 'color-theme', 'pp_dark'].includes(e.key)) {
        try {
          const val =
            localStorage.getItem('theme') ||
            (localStorage.getItem('pp_dark') === '1' ? 'dark' : 'light');
  ...
  // 👇 No "buy a plan" banner until LIVE subscription check is done.
  const showNoPlanUI = authChecked && !!user && !hasActivePlan && subChecked;
